...
class Updatable extends Events {
  ...
  async _getUpdateCommits (manifestOnly) {
    ...
    let gitLogCmd = `git log --format="%H -- %an -- %s" ..origin/${branch}`
    if (manifestOnly) {
  ...
  async _getUpdateVersion () {
    const latest = await this._getUpdateCommits(true);
    if (latest[0]) {
      try {
        const gitManifest = await PowercordNative.exec(`git show ${latest[0].id}:${this.manifestFilename}`, {
          cwd: this.entityPath,
          timeout: TIMEOUT
        }).then(({ stdout }) => stdout.toString());
