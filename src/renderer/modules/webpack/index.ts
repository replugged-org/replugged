...
export const waitForStart = new Promise<void>(
	resolve => (signalStart = resolve),
);
	...
	const patchedSource = plaintextPatches.reduce((source, patch) => {
		if (
			...
			!(typeof patch.find === 'string'
				? source.includes(patch.find)
				: patch.find.test(source))
		) {
			return source;
		}

		const result = patch.replacements.reduce(
...
function getExports(m: RawModule): ModuleExports | undefined {
	if (typeof m.exports === 'object') {
		...
		if (
			exportKeys.length === 1 &&
			['default', 'Z'].includes(exportKeys[0])
		) {
...
function getExportsForProps<P extends string>(
	...
	if (typeof m.exports === 'object') {
		return [m.exports, ...Object.values(m.exports)].find(
			o =>
				typeof o === 'object' && o !== null && props.every(p => p in o),
...
export function byPropsFilter<P extends string>(props: P[]) {
	return (m: RawModule): m is RawModuleWithProps<P> =>
		Boolean(getExportsForProps(m, props));
}
...
export function getRawByProps<P extends string>(
	...props: P[]
): RawModuleWithProps<P> | undefined {
	return getRawModule(byPropsFilter(props)) as
		| RawModuleWithProps<P>
		| undefined;
}
...
export function getAllByProps<P extends string>(
	...props: P[]
): Array<ModuleExportsWithProps<P>> {
	return byPropsInternal(props, true);
}

export function getAllRawByProps<P extends string>(
	...props: P[]
): Array<RawModuleWithProps<P>> {
	return getAllRawModules(byPropsFilter(props)) as Array<
		RawModuleWithProps<P>
	>;
}
