...
export default () => {
  ...
  if (
    __dirname.toLowerCase().replace(/\\/g, "/").includes("/windows/system32")
  ) {
  ...
  if (!existsSync(nodeModulesPath)) {
    ...
    for (const dependency in dependencies) {
      ...
      const expectedVerInt = parseInt(
        dependencies[dependency].replace(/[^\d]/g, "")
      );
      const installedVerInt = parseInt(
        depPackage.version.replace(/[^\d]/g, "")
      );
